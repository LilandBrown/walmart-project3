#!/usr/bin/env python3
"""
Walmart Toolkit — the cart tool and the checkout demo as one tool, on one connection.

    Session          one connection: adb, frida, the instance picker, the shared log
                     and Settings (session.py)
    CartPanel        intercept add-to-cart, edit, re-send, presets, timed send
    CheckoutPanel    observe and replay CreatePurchaseContract / PlaceOrder

There is ONE Frida agent (agent_toolkit.js) hooking Apollo's composer once and
dispatching on the operation name, so both halves are live on the same instance at the
same time — capture an add-to-cart and walk checkout without reconnecting or switching
anything. That is what the tabbed build could not do: two agents cannot both own one
Java method.

LAYOUT
Everything lives on one scrolling page rather than behind tabs, so a section can be
collapsed to its header and the parts you are presenting can sit on screen together.
Click a section header (or its ▸/▾) to fold it. `Collapse all` / `Expand all` are in the
toolbar, and the layout you leave it in is remembered in config.json.
"""
import json
import os
import sys
...
from session import Session, save_json, CONFIG_PATH
import cart_panel
import checkout_panel

BASE = os.path.dirname(sys.executable) if getattr(sys, "frozen", False) \
    else os.path.dirname(os.path.abspath(__file__))

AGENT_FILE = os.path.join(BASE, "agent_toolkit.js")
# Embedded at build time by embed_agents.py so the exe is self-contained. The .js next
# to the exe wins if present, which is how an agent change is tried without rebuilding.
# tests/t_merge.py fails the build if this drifts from agent_toolkit.js — the cart tool
# shipped a 7,000-char-stale embedded agent for exactly that lack of a check.
AGENT_JS = r"""'use strict';
...
def load_agent():
    """The agent source: the file next to us if there is one, else the embedded copy."""
    for path in (AGENT_FILE, os.path.join(getattr(sys, "_MEIPASS", BASE), "agent_toolkit.js")):
        try:
...
class Section(ttk.Frame):
    """A collapsible block: a clickable header and a body that folds away.

    Collapsing is `pack_forget` on the body rather than a height animation — the body
    holds real widgets whose geometry must survive being hidden and shown, and tk is
    happy to re-pack them exactly as they were.
    """

    def __init__(self, parent, title, subtitle="", open_=True, on_toggle=None):
        super().__init__(parent)
        self.title = title
        self._open = bool(open_)
        self._on_toggle = on_toggle

        self.header = ttk.Frame(self, padding=(6, 3))
        self.header.pack(fill="x")
        self.arrow = ttk.Label(self.header, width=2,
                               text="▾" if self._open else "▸")
        self.arrow.pack(side="left")
        self.label = ttk.Label(self.header, text=title,
                               font=("Segoe UI", 10, "bold"))
        self.label.pack(side="left")
        self.sub = ttk.Label(self.header, text=subtitle, foreground="#6b6b6b")
        self.sub.pack(side="left", padx=8)
        ttk.Separator(self, orient="horizontal").pack(fill="x")

        self.body = ttk.Frame(self, padding=(4, 2))
        if self._open:
            self.body.pack(fill="both", expand=True)

        # The whole header is the hit target, not just the arrow — a 2-character
        # chevron is a fussy thing to aim at mid-presentation.
        for w in (self.header, self.arrow, self.label, self.sub):
    ...
    def set_open(self, open_):
        ...
        if open_ == self._open:
            return
        self._open = open_
        self.arrow.config(text="▾" if open_ else "▸")
        if open_:
            self.body.pack(fill="both", expand=True)
        else:
...
class ScrollPage(ttk.Frame):
    """A vertically scrolling page. Everything goes in `.inner`.

    A tk Canvas is the only way to scroll arbitrary widgets. Two bindings make it behave
    like a page rather than a canvas: the inner frame is kept at the canvas's width so
    nothing scrolls sideways, and the wheel is bound on <Enter>/<Leave> rather than
    globally — a global bind_all steals the wheel from the log pane and every listbox
    inside it, which is exactly the content you want to scroll independently.
    """
    ...
    def _on_wheel(self, e):
        # Don't hijack a scrollable widget the pointer is actually over (the log, the
        # capture lists): let it consume its own wheel events.
        w = self.winfo_containing(e.x_root, e.y_root)
        while w is not None and w is not self.canvas:
            if isinstance(w, (tk.Text, tk.Listbox, tk.Canvas)) and w is not self.canvas:
                return
...
class Toolkit:
    # Which sections start folded, when config.json has nothing to say. The cart's
    # capture/editor work is the usual starting point; the timing panel and the
    # checkout half are opened when they are being shown.
    DEFAULT_OPEN = {"instances": True, "cart": True, "checkout": False, "log": True}

    def __init__(self, root):
        self.root = root
        root.title("Walmart Toolkit")
        # Wider than the 980 the tabbed build opened at: the Cart section's middle row
        # is three boxes now (intercepted | sent requests | item editor) and wants
        # ~1100 to show all three at their natural width. Narrower still works — the
        # editor is packed first so it keeps its size and the two lists give up the
        # pixels — but there is no reason to launch already squeezed.
        # Taller than the 1000 the single-connection build opened at: the picker is
        # six rows now rather than one or two, and the whole point of the page is that
        # you can see the accounts and their captures without scrolling between them.
        root.geometry("1180x1060")
        root.minsize(860, 560)
        self._set_icon(root)

        self.session = Session(root, agent_js=load_agent())

        # --- toolbar: the one connection, plus the section controls ---
        bar = ttk.Frame(root)
        bar.pack(fill="x")
        self.session.build_top_bar(bar).pack(side="left", fill="x", expand=True)
        ttk.Button(bar, text="Collapse all", width=12,
                   command=lambda: self._all(False)).pack(side="right", padx=(0, 6))
        ttk.Button(bar, text="Expand all", width=11,
                   command=lambda: self._all(True)).pack(side="right", padx=4)
        ttk.Separator(root, orient="horizontal").pack(fill="x")

        # --- the scrolling page ---
        self.page = ScrollPage(root)
        self.page.pack(fill="both", expand=True)
        open_ = self._load_open()
        self.sections = {}

        def section(key, title, subtitle):
            s = Section(self.page.inner, title, subtitle,
                        open_=open_.get(key, self.DEFAULT_OPEN.get(key, True)),
                        on_toggle=self._toggled)
            s.pack(fill="x", pady=(0, 2))
            self.sections[key] = s
            return s

        inst = section("instances", "Emulator instances",
                       "tick the accounts to use — up to six attach at once, and every "
                       "row below names the one it came from")
        self.session.build_instance_panel(inst.body).pack(fill="x")

        cart = section("cart", "Cart",
                       "intercept add-to-cart, edit it, re-send it, presets, timed send")
        self.cart = cart_panel.CartPanel(cart.body, self.session)

        chk = section("checkout", "Checkout",
                      "observe and replay CreatePurchaseContract / PlaceOrder")
        self.checkout = checkout_panel.CheckoutPanel(chk.body, self.session)

        # Every mode in one panel, side by side. The checkout half's two selectors are
        # built as further columns of the Cart section's Mode strip rather than living
        # in its own section, so the switch that decides whether a PlaceOrder reaches
        # Walmart is never hidden inside a section you happened to collapse.
        self.checkout.build_modes(self.cart.mode_cols)

        log = section("log", "Log", "both halves, in the order things happened")
        self.session.build_log(log.body).pack(fill="both", expand=True)

        # One pump for the session's own queue; each panel drains its own.
        root.after(300, self.session._pump)
        # Detect once, for both halves — this is what used to happen twice.
        root.after(200, lambda: self.session.detect(quiet=True))
    ...
    def _load_open(self):
        """Remembered layout. A malformed value must not stop the tool opening."""
        try:
            return json.loads(self.session.cfg.get("sections_open") or "{}")
        except Exception:
            return {}

    def _save_open(self):
        self.session.cfg["sections_open"] = json.dumps(
            {k: s.is_open for k, s in self.sections.items()})
        try:
            save_json(CONFIG_PATH, self.session.cfg)
        except Exception:
            pass          # a read-only dir must not break folding a section

    # ---------- window ----------
    def _set_icon(self, root):
        try:
            for base in (getattr(sys, "_MEIPASS", None), BASE):
                ico = base and os.path.join(base, "icon.ico")
    ...
    def _on_close(self):
        """Drop the session before the window goes, so no agent is left hooked and no
        adb forward stays bound for the next run to trip over."""
        try:
